...
describe('createBooking', () => {
  it('returns 400 when package is not active', async () => {
    const supabase = makeSupabase({
      packages: {
        selectResult: {
          data: { id: 'pkg-1', name: 'pkg', base_price_cents: 10000, events_count: 1, is_active: false },
          error: null,
        },
      },
    });
  ...
  it('returns 400 when events exceed package events_count', async () => {
    ...
    const supabase = makeSupabase({
      packages: {
        selectResult: {
          data: { id: 'pkg-1', name: 'pkg', base_price_cents: 10000, events_count: 1, is_active: true },
          error: null,
        },
      },
    });
