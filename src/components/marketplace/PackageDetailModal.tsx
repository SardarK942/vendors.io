    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          {pkg.gallery_image_urls.length > 0 && (
            <div className="grid grid-cols-2 gap-2">
              {pkg.gallery_image_urls.map((url, idx) => (
                <div key={idx} className="relative aspect-[4/3] rounded overflow-hidden bg-muted">
                  <Image
                    src={url}
                    alt={`Gallery ${idx + 1}`}
                    fill
          ...
          {pkg.addons.length > 0 && (
            <div>
              <h4 className="font-semibold mb-2 text-sm">Add-ons (optional)</h4>
              <div className="space-y-2">
                {pkg.addons.map((addon) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-mono ${addon.price_delta_cents < 0 ? 'text-green-600' : ''}`}
                    >
          ...
          {pkg.vendor_notes_template && (
            <div className="text-xs italic text-muted-foreground p-3 rounded-lg bg-muted">
