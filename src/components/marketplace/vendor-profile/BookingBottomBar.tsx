...
export function BookingBottomBar({
  ...
  if (!selected || selected.base_price_cents == null) {
    return (
      <div
        ...
        className="js-booking-bottom-bar fixed bottom-0 left-0 right-0 z-50 touch-manipulation bg-white px-4 py-2.5 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] md:hidden"
  ...
  return (
    <div
      data-testid="vendor-bottom-bar"
      className="js-booking-bottom-bar fixed bottom-0 left-0 right-0 z-50 bg-white px-4 py-2.5 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] md:hidden"
      style={{ paddingBottom: `calc(0.625rem + env(safe-area-inset-bottom))` }}
    >
      <div className="flex items-center justify-between gap-3">
